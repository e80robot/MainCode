...
void loop() {
  ...
  if ( currentTime-pcontrol.lastExecutionTime >= LOOP_PERIOD ) {
    ...
    motor_driver.drive(int(pcontrol.uL),int(pcontrol.uR),int(pcontrol.uV));
    Serial.println("Running the proper loop");
    Serial.println("uV is");
    Serial.println(pcontrol.uV);
  }
